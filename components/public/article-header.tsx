    ...
    <header className="flex flex-col gap-6 border-b border-[var(--public-border-strong)] pb-8 sm:gap-7 sm:pb-10">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[0.68rem] font-extrabold tracking-[0.1em] uppercase">
        <Link
          ...
          aria-label={`View more articles in ${article.category.name}`}
          className="border-l-[3px] border-[var(--public-accent)] pl-2 text-[var(--public-accent)] hover:underline"
        >
          {article.category.name}
        </Link>
        {article.isBreaking ? (
          <span className="bg-[var(--public-accent)] px-2 py-1 text-white">Breaking news</span>
        ) : null}
        {article.isFeatured ? (
          <span className="border border-[var(--public-border-strong)] px-2 py-1">Featured</span>
        ) : null}
      </div>
