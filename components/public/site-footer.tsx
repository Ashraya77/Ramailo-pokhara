...
export function SiteFooter({ categories }: SiteFooterProps) {
  return (
    <footer className="mt-auto border-t-[3px] border-[var(--public-accent)] bg-[var(--public-ink)] text-[var(--public-paper)]">
      <div className="public-container flex flex-col gap-10 py-12 sm:py-16">
        <div className="grid gap-9 md:grid-cols-[minmax(0,1.2fr)_minmax(0,2fr)]">
          ...
          <nav aria-label="Footer navigation">
            <p className="mb-4 text-[0.68rem] font-bold tracking-[0.14em] text-white/45 uppercase">Explore</p>
            <ul className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm sm:grid-cols-3">
              {siteConfig.navigation.map((item) => (
                <li key={item.href}>
                  <Link className="text-white/75 underline-offset-4 hover:text-white hover:underline" href={item.href}>
                    {item.label}
                  </Link>
                </li>
              ))}
        ...
        <p className="border-t border-white/15 pt-6 text-xs tracking-wide text-white/45">
          © {new Date().getFullYear()} {siteConfig.name}. Independent reporting from Pokhara.
        </p>
      </div>
    </footer>
  );
