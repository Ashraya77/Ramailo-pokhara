...
const quickLinks = [
  ...
  { label: "खेलकुद", href: "/category/khelkud" },
  { label: "मनोरञ्जन", href: "/category/manoranjan" },
  { label: "विचार", href: "/news" },
  { label: "हाम्रो बारे", href: "/about-us" },
] as const;
...
function FooterColumnTitle({ children }: Readonly<{ children: React.ReactNode }>) {
  return (
    <p className="text-[0.72rem] font-bold tracking-[0.18em] text-[var(--public-accent)] uppercase">
      {children}
    </p>
  );
}
...
export async function SiteFooter({ categories }: SiteFooterProps) {
  const highlightedCategories = categories.filter((category) =>
    ["samachar", "rajniti", "khelkud", "manoranjan"].includes(category.slug),
  );
    ...
    <footer className="mt-auto border-t-4 border-[var(--public-accent)] bg-[var(--public-ink)] text-[var(--public-background)]">
      <div className="public-container py-12 sm:py-16">
        <div className="grid grid-cols-2 gap-8 text-center md:grid-cols-2 md:gap-10 md:text-left xl:grid-cols-5">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 md:justify-start">
              {socialItems.map(({ label, href, icon: Icon }) => (
                <Link
                  ...
                  href={href}
                  aria-label={label}
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-white/12 bg-white/5 text-white transition hover:border-[var(--public-accent)] hover:text-[var(--public-accent)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white"
                >
          ...
          <nav
            aria-label="फुटर द्रुत लिङ्क"
            className="xl:px-6 xl:border-r xl:border-white/10"
          >
            <FooterColumnTitle>द्रुत लिङ्क</FooterColumnTitle>
            <ul className="mt-5 grid gap-3 text-sm">
              {quickLinks.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-white/80 no-underline transition hover:text-[var(--public-accent)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white"
                  >
                    {item.label}
                  </Link>
                </li>
              ...
              {highlightedCategories
                .filter(
                  (category) =>
                    !quickLinks.some(
                      (item) => item.href === `/category/${encodeURIComponent(category.slug)}`,
                    ),
                )
                .map((category) => (
            ...
            <ul className="mt-5 space-y-4 text-sm">
              {contactDetails.map(({ label, value, href, icon: Icon }) => (
                <li key={label}>
                  <a
                    href={href}
                    className="flex items-start justify-center gap-3 text-white/80 no-underline transition hover:text-[var(--public-accent)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white md:justify-start"
                  >
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-6 text-center text-xs text-white/50 sm:flex-row sm:text-left">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 sm:justify-end">
            <Link
              href="/privacy-policy"
              className="text-white/70 no-underline transition hover:text-[var(--public-accent)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white"
            >
              Privacy Policy
            </Link>
            <Link
